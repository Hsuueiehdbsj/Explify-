...
A Streamlit app for summarizing Text, Images, and Videos using Google Gemini
"""
...
if 'model_name' not in st.session_state:
    if api_configured:
        try:
            ...
            if not st.session_state.model_name and available_models:
...
with st.sidebar:
    ...
    Explify uses **Google Gemini AI** to provide instant, intelligent summaries of:
    - 📄 Text documents
    - 🖼️ Images
    - 🎥 Videos
    ...
    **Powered by AI** - No API key needed! 🚀
    ...
    if st.session_state.api_configured:
        model_display = st.session_state.model_name.replace('models/', '')
        st.markdown(f"<small>Using: {model_display}</small>", unsafe_allow_html=True)
...
st.markdown("---")
st.markdown(
    '<p style="text-align: center; color: #6b7280; font-size: 0.875rem;">Made with ⚡ by Explify | Powered by Google Gemini AI</p>',
    unsafe_allow_html=True
)
